...
void audio_record_init()
{
    ...
    ESP_LOGI(TAG, "successfully created record_audio_buffer with a size: %zu", FILE_SIZE);

#endif

    if (record_audio_buffer == NULL)
    {
        ESP_LOGE(TAG, "Error: Failed to allocate memory for buffers");
...
void sr_handler_task(void *pvParam)
{
    ...
    while (true)
    {
        ...
        if (ESP_MN_STATE_TIMEOUT == result.state)
        {
            ...
            {
                ESP_LOGI(TAG, "APP_AUDIO_START");
                ESP_LOGI(TAG, "record_audio_buffer=%s", (const char *)record_audio_buffer);
                app_baidu_iar_https_post((const char *)record_audio_buffer, record_total_len);
            }
            continue;
        }
